...
import com.study.todopartytest.todo.TodoRepository;
...
class TodoServiceTest implements TodoTest {
    ...
    void getUserTodoMap() {
        ...
        var testTodo2 = TodoTestUtils.get(TEST_TODO, 2L, LocalDateTime.now().minusMinutes(1),
            TEST_USER);
        var testAnotherTodo = TodoTestUtils.get(TEST_TODO, 1L, LocalDateTime.now(),
            TEST_ANOTHER_USER);

        given(todoRepository.findAll(any(Sort.class))).willReturn(
            List.of(testTodo1, testTodo2, testAnotherTodo));

        // when
        var result = todoService.getUserTodoMap();

        // then
        assertThat(result.get(new UserDTO(TEST_USER)).get(0)).isEqualTo(
            new TodoResponseDTO(testTodo1));
        assertThat(result.get(new UserDTO(TEST_USER)).get(1)).isEqualTo(
            new TodoResponseDTO(testTodo2));
        assertThat(result.get(new UserDTO(TEST_ANOTHER_USER)).get(0)).isEqualTo(
            new TodoResponseDTO(testAnotherTodo));
    }
    ...
    class getUserTodo {
        ...
        void getUserTodo_fail() {
            ...
            var testTodo = TodoTestUtils.get(TEST_TODO, TEST_USER);
            given(todoRepository.findById(eq(TEST_TODO_ID))).willReturn(Optional.of(testTodo));
            ReflectionTestUtils.setField(TEST_ANOTHER_USER, User.class, "id", TEST_ANOTHER_USER_ID,
                Long.class);

            // when & exception
            assertThrows(RejectedExecutionException.class,
                () -> todoService.getUserTodo(TEST_TODO_ID, TEST_ANOTHER_USER));
        }
    }
}
