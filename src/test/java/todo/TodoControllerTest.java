...
import com.study.todoPartyTest.entity.ControllerTest;
import com.study.todoPartyTest.entity.TodoTest;
import com.study.todoPartyTest.entity.TodoTestUtils;
import com.study.todopartytest.todo.TodoRequestDTO;
import com.study.todopartytest.todo.TodoResponseDTO;
import com.study.todopartytest.todo.TodoService;
...
class TodoControllerTest extends ControllerTest implements TodoTest {
    ...
    class getTodo {
        ...
        void getTodo_success() throws Exception {
            ...
            //when
            var action = mockMvc.perform(get("/qpi/todos/{todoId}", TEST_TODO_ID)
                .accept(MediaType.APPLICATION_JSON));
    ...
    void getTodoList() throws Exception {
        ...
        var testTodo2 = TodoTestUtils.get(TEST_TODO, 2L, LocalDateTime.now().minusMinutes(1),
            TEST_USER);
        var testAnotherTodo = TodoTestUtils.get(TEST_TODO, 3L, LocalDateTime.now(),
            TEST_ANOTHER_USER);

        given(todoService.getUserTodoMap()).willReturn(
            Map.of(new UserDTO(TEST_USER),
        ...
        action
            .andExpect(status().isOk())
            .andExpect(
                jsonPath("$[?(@.user.username=='" + TEST_USER.getUsername() + "')].todoList[*].id")
                    .value(Matchers.containsInAnyOrder(testTodo1.getId().intValue(),
                        testTodo2.getId().intValue())))
            .andExpect(jsonPath(
                "$[?(@.user.username=='" + TEST_ANOTHER_USER.getUsername() + "')].todoList[*].id")
    ...
    class putTodo {
        ...
        void putTodo_success() throws Exception {
            // given
            given(todoService.updateTodo(eq(TEST_TODO_ID), eq(TEST_TODO_REQUEST_DTO),
                any(User.class))).willReturn(TEST_TODO_RESPONSE_DTO);
        ...
        void putTodo_fail_rejected() throws Exception {
            // given
            given(todoService.updateTodo(eq(TEST_TODO_ID), eq(TEST_TODO_REQUEST_DTO),
                any(User.class))).willThrow(new RejectedExecutionException());
    ...
    class completeTodo {
        ...
        void completeTodo_success() throws Exception {
            ...
            given(todoService.completeTodo(eq(TEST_TODO_ID), any(User.class))).willReturn(
                TEST_TODO_RESPONSE_DTO);
        ...
        void completeTodo_fail_rejected() throws Exception {
            // given
            given(todoService.completeTodo(eq(TEST_TODO_ID), any(User.class))).willThrow(
        ...
        @DisplayName("할 일 완료 요청 실패 - 존재하지 않는 할일ID")
        @Test
